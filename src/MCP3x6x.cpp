// SPDX-License-Identifier: MIT

/**
 * @file MCP3x6x.cpp
 * @author Stefan Herold (stefan.herold@posteo.de)
 * @brief
 * @version 0.0.2
 * @date 2023-04-08
 *
 * @copyright Copyright (c) 2023
 *
 */
#include "MCP3x6x.hpp"

#include <Arduino.h>

#include "MCPChannel.hpp"
#include "MCPSettings.hpp"

#if ARDUINO_ARCH_SAMD
#  include <wiring_private.h>
#endif

MCP3x6x::MCP3x6x(const uint8_t pinCS, SPIClass *theSPI, const uint8_t pinMOSI,
                 const uint8_t pinMISO, const uint8_t pinCLK, MCPChannel *channel) {
  _spi     = theSPI;
  _pinMISO = pinMISO;
  _pinMOSI = pinMOSI;
  _pinCLK  = pinCLK;
  _pinCS   = pinCS;
  _channel = channel;
}

MCP3x6x::MCP3x6x(const uint8_t pinIRQ, const uint8_t pinMCLK, const uint8_t pinCS, SPIClass *theSPI,
                 const uint8_t pinMOSI, const uint8_t pinMISO, const uint8_t pinCLK,
                 MCPChannel *channel)
    : MCP3x6x(pinCS, theSPI, pinMOSI, pinMISO, pinCLK, channel) {
  _pinIRQ  = pinIRQ;
  _pinMCLK = pinMCLK;
}

void MCP3x6x::_reverse_array(uint8_t *array, size_t size) {
  for (size_t i = 0, e = size; i <= e / 2; i++, e--) {
    uint8_t temp = array[i];
    array[i]     = array[e - 1];
    array[e - 1] = temp;
  }
}

MCP3x6x::status_t MCP3x6x::_transfer(uint8_t *data, uint8_t addr, size_t size) {
  _spi->beginTransaction(SPISettings(MCP3x6x_SPI_SPEED, MCP3x6x_SPI_ORDER, MCP3x6x_SPI_MODE));
  noInterrupts();
  digitalWrite(_pinCS, LOW);
  _status.raw = _spi->transfer(addr);
  _spi->transfer(data, size);
  digitalWrite(_pinCS, HIGH);
  interrupts();
  _spi->endTransaction();
  return _status;
}

/*
bool MCP3x6x::begin(uint16_t channelmask, float vref) {
  pinMode(_pinCS, OUTPUT);
  digitalWrite(_pinCS, HIGH);

  _spi->begin();

#if ARDUINO_ARCH_SAMD
  // todo figure out how to get dynamicaly sercom index
  pinPeripheral(_pinMISO, PIO_SERCOM);
  pinPeripheral(_pinMOSI, PIO_SERCOM);
  pinPeripheral(_pinCLK, PIO_SERCOM);
#endif

  _status = reset(true);
  setClockSelection(MCPSettings::clk_sel::INTERN);  // todo make configurable by function parameter
  setDataFormat(
      MCPSettings::data_format::ID_SGNEXT_DATA);  // todo make configurable by function parameter

  // scanmode
  if (channelmask != 0) {
    settings.scan.channel.raw = channelmask;  // todo apply _channel_mask
    _status                   = write(settings.scan);
  }

  setReference(vref);
  _status = standby();

  return true;
}
*/
bool MCP3x6x::begin(MCPSettings settings) {
  pinMode(_pinCS, OUTPUT);
  digitalWrite(_pinCS, HIGH);

  _spi->begin();

#if ARDUINO_ARCH_SAMD
  // todo figure out how to get dynamicaly sercom index
  pinPeripheral(_pinMISO, PIO_SERCOM);
  pinPeripheral(_pinMOSI, PIO_SERCOM);
  pinPeripheral(_pinCLK, PIO_SERCOM);
#endif

  _status        = reset();
  this->settings = settings;
  //  _status        = write(settings);

  return true;
}

MCP3x6x::status_t MCP3x6x::read(MCPChannel::Adcdata *data) {
  size_t s = 0;

  switch (_channel->_resolution) {
    case 16:
      s = settings.config3.data_format == MCPSettings::data_format::SGN_DATA ? 2 : 4;
      break;
    case 24:
      s = settings.config3.data_format == MCPSettings::data_format::SGN_DATA ? 3 : 4;
      break;
  }

  uint8_t buffer[s];
  _status = _transfer(buffer, MCP3x6x_CMD_SREAD | MCP3x6x_ADR_ADCDATA, s);
  _reverse_array(buffer, s);

#if MCP3x6x_DEBUG
  Serial.print("buffer: 0x");
  Serial.print(buffer[3], HEX);
  Serial.print(buffer[2], HEX);
  Serial.print(buffer[1], HEX);
  Serial.println(buffer[0], HEX);
#endif

  data->channelid = _getChannel((uint32_t &)buffer);
  data->value     = _getValue((uint32_t &)buffer);

  return _status;
}

void MCP3x6x::IRQ_handler() {
  do {
    _status = read(&_channel->_adcdata);
  } while (!_status.dr);

  _channel->result.raw[_channel->_adcdata.channelid] = _channel->_adcdata.value;

#if MCP3x6x_DEBUG
  Serial.print("channel: ");
  Serial.println(_channel->_adcdata.channelid);
  Serial.print("value: ");
  Serial.println(_channel->_adcdata.value);
#endif
}

void MCP3x6x::lock(uint8_t key) {
  settings.lock.raw = key;
  _status           = write(settings.lock);
}

void MCP3x6x::unlock() {
  settings.lock.raw = MCPSettings::_DEFAULT_LOCK;
  _status           = write(settings.lock);
}

void MCP3x6x::setDataFormat(MCPSettings::data_format format) {
  settings.config3.data_format = format;
  _status                      = write(settings.config3);

  switch (format) {
    case MCPSettings::data_format::SGN_DATA:
    case MCPSettings::data_format::SGN_DATA_ZERO:
      _channel->_resolution--;
      break;
    case MCPSettings::data_format::SGNEXT_DATA:
    case MCPSettings::data_format::ID_SGNEXT_DATA:
      break;
    default:
      _channel->_resolution = -1;
      break;
  }
#if MCP3x6x_DEBUG
  Serial.print("resolution");
  Serial.println(_channel->_resolution);
#endif
}

void MCP3x6x::setConversionMode(MCPSettings::conv_mode mode) {
  settings.config3.conv_mode = mode;
  _status                    = write(settings.config3);
}

void MCP3x6x::setAdcMode(MCPSettings::adc_mode mode) {
  settings.config0.adc = mode;
  _status              = write(settings.config0);
}

void MCP3x6x::setClockSelection(MCPSettings::clk_sel clk) {
  settings.config0.clk = clk;
  _status              = write(settings.config0);
}

void MCP3x6x::enableScanChannel(MCPSettings::mux_t ch) {
  for (size_t i = 0; i < sizeof(MCPChannel::_channelID); i++) {
    if (_channel->_channelID[i] == ch.raw) {
      bitSet(settings.scan.channel.raw, i);
#ifdef MCP3x6x_DEBUG
      Serial.println(i);
      Serial.println(ch.raw, HEX);
      Serial.println(settings.scan.channel.raw, HEX);
#endif
      break;
    }
  }
  _status = write(settings.scan);
}

void MCP3x6x::disableScanChannel(MCPSettings::mux_t ch) {
  for (size_t i = 0; i < sizeof(MCPChannel::_channelID); i++) {
    if (_channel->_channelID[i] == ch.raw) {
      bitClear(settings.scan.channel.raw, i);
      break;
    }
  }
  _status = write(settings.scan);
}

void MCP3x6x::setReference(float vref) {
  if (vref == 0.0) {
    vref                      = 2.4;
    settings.config0.vref_sel = 1;
    _status                   = write(settings.config0);
  }
  _reference = vref;
}

size_t MCP3x6x::getResolution() { return _channel->_resolution; }

float MCP3x6x::getReference() { return _reference; }

// returns signed ADC value from raw data
int32_t MCP3x6x::_getValue(uint32_t raw) {
#if 1
  switch (settings.config3.data_format) {
    case (MCPSettings::data_format::SGN_DATA_ZERO):
      return raw >> 16;
    case (MCPSettings::data_format::SGN_DATA):
      bitWrite(raw, 31, bitRead(raw, 16));
      bitClear(raw, 16);
      return raw;
    case (MCPSettings::data_format::SGNEXT_DATA):
    case (MCPSettings::data_format::ID_SGNEXT_DATA):
      bitWrite(raw, 31, bitRead(raw, 17));
      return raw & 0x8000FFFF;
  };

  return -1;
#else
  switch (settings.config3.data_format) {
    case (MCPSettings::data_format::SGN_DATA_ZERO):
      return raw >> 8;
    case (MCPSettings::data_format::SGN_DATA):
      bitWrite(raw, 31, bitRead(raw, 24));
      bitClear(raw, 24);
      return raw;
    case (MCPSettings::data_format::SGNEXT_DATA):
    case (MCPSettings::data_format::ID_SGNEXT_DATA):
      bitWrite(raw, 31, bitRead(raw, 25));
      return raw & 0x80FFFFFF;
  }
  return -1;
#endif
}

uint8_t MCP3x6x::_getChannel(uint32_t raw) {
  if (settings.config3.data_format == MCPSettings::data_format::ID_SGNEXT_DATA) {
    return ((raw >> 28) & 0x0F);
  } else {
    for (size_t i = 0; i < sizeof(MCPChannel::_channelID); i++) {
      if (_channel->_channelID[i] == settings.mux.raw) {
        return i;
      }
    }
  }
  return -1;
}

int32_t MCP3x6x::analogRead(MCPSettings::mux_t ch) {
  // MuxMode
  if (settings.scan.channel.raw == 0) {
#ifdef MCP3x6x_DEBUG
    Serial.println("mux");
#endif
    settings.mux = ch;
    _status      = write(settings.mux);
    _status      = conversion();

    do {
      _status = read(&_channel->_adcdata);
    } while (!_status.dr);

    return _channel->result.raw[_channel->_adcdata.channelid] = _channel->_adcdata.value;
  }

#ifdef MCP3x6x_DEBUG
  Serial.println("scan");
#endif
  // ScanMode
  for (size_t i = 0; i < sizeof(MCPChannel::_channelID); i++) {
    if (_channel->_channelID[i] == ch.raw) {
      _status = conversion();

      do {
        _status = read(&_channel->_adcdata);
      } while (!_status.dr);

      return _channel->_adcdata.value;
    }
  }
  return -1;
}

int32_t MCP3x6x::analogReadDifferential(enum MCPSettings::mux pinP, enum MCPSettings::mux pinN) {
  //  settings.mux = ((uint8_t)pinP << 4) | (uint8_t)pinN;
  write(settings.mux);

  _status                                                   = conversion();
  _status                                                   = read(&_channel->_adcdata);
  return _channel->result.raw[_channel->_adcdata.channelid] = _channel->_adcdata.value;
}

void MCP3x6x::analogReadResolution(size_t bits) {
  if (bits <= _channel->_RESOLUTION_MAX) {
    _channel->_resolution = bits;
  }
}

void MCP3x6x::setResolution(size_t bits) { analogReadResolution(bits); }

void MCP3x6x::singleEndedMode() { _differential = false; }

void MCP3x6x::differentialMode() { _differential = true; }

bool MCP3x6x::isDifferential() { return _differential; }

uint32_t MCP3x6x::getMaxValue() { return pow(2, _channel->_resolution); }

bool MCP3x6x::isComplete() { return _status.dr; }

void MCP3x6x::startContinuous() {
  //  attachInterrupt(digitalPinToInterrupt(_pinIRQ), mcp_wrapper, FALLING);

  setConversionMode(MCPSettings::conv_mode::CONTINUOUS);
  conversion();
}

void MCP3x6x::stopContinuous() {
  detachInterrupt(digitalPinToInterrupt(_pinIRQ));

  setConversionMode(MCPSettings::conv_mode::ONESHOT_STANDBY);
  standby();
}

void MCP3x6x::startContinuousDifferential() {
  differentialMode();
  startContinuous();
}

bool MCP3x6x::isContinuous() {
  if (settings.config3.conv_mode == MCPSettings::conv_mode::CONTINUOUS) {
    return true;
  }
  return false;
}

void MCP3x6x::setAveraging(MCPSettings::osr rate) {
  settings.config1.osr = rate;
  _status              = write(settings.config1);
}

void MCP3x6x::setGain(MCPSettings::gain gain) {
  settings.config2.gain = gain;
  _status               = write(settings.config2);
}

int32_t MCP3x6x::analogReadContinuous(MCPSettings::mux_t ch) {
  if (isContinuous()) {
    for (size_t i = 0; i < sizeof(MCPChannel::_channelID); i++) {
      if (_channel->_channelID[i] == ch.raw) {
        return _channel->result.raw[_channel->_adcdata.channelid];
      }
    }
  }
  return -1;
}
